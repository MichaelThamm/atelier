...
The design targets a **development trust model with no sensitive secrets in
scope.** This ADR documents that posture explicitly so it is not forgotten
when the project graduates to production use cases.

## Decision

Atelier implements **(α): variable indirection with gitignored
`secrets.auto.tfvars`**, with the following explicit limitations.

### Limitations (read this carefully)

1. **Development-environment trust model.** Atelier assumes the user is
   operating in a single-user development environment on a machine they own
   and trust.
...
3. **No env-var alternative yet.** Option (β) is not implemented. Users who
   want env-var-based secret injection can hand-edit `providers.tf` and not
   use the TUI's secret editor for those fields.
4. **No external secret managers.** Vault, AWS Secrets Manager, cloud
   KMS integration, age/sops encrypted files — all out of scope. Atelier
   does not provide an abstraction over secret backends.
5. **No per-field source toggle.** Option (γ) is not yet implemented (see
   [ROADMAP](../ROADMAP.md)).
...
### Atelier's actual responsibilities
...
(γ) hybrid is the right future target — let the user opt-in per field to env-var
mode — but it does not exist yet.

## Consequences

- **The README explicitly warns** that Atelier's secret handling is
  ...
  (b) an external secret backend the user wires up themselves, (c) a future
  Atelier release.
- Future work includes (γ) hybrid per-field choice. The wrapper format need
  not change to accommodate it; the variable indirection pattern is the same,
  only the *source* of the value differs.
- A later release may add explicit integrations with secret backends. That
