...
type RefSwitcher interface {
	...
	// ListRefs returns the human-friendly ref names (branches and tags) the
	// module's remote currently has, for presentation in the ref-switch
	// modal. It is the source of truth for the "available refs" hint so the
	// list is never stale — including after an in-session switch fails or a
	// ref is deleted mid-session. Returns an error when the remote can't be
	// reached; callers treat that as non-fatal (the input stays free-text).
