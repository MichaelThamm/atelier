...
// generatedHeader is prepended to atelier.local.yaml files Atelier creates via
// SavePreset. It documents provenance and Atelier's hands-off contract: the
// file is created once and never modified again.
const generatedHeader = `# atelier.local.yaml — user-owned, wrapper-local Atelier presets.
#
# Generated by Atelier (Save preset) from this wrapper's current configuration.
# Atelier will not modify this file again: Save preset refuses to overwrite an
# existing atelier.local.yaml. Edit it freely, add more presets by hand, or
# move it to a parent directory to share it across sibling wrappers.
#
# See docs/SPEC.md §11 and docs/adr/0022-local-presets.md.
...
// HasLocalFile reports whether an atelier.local.yaml already exists directly in
// dir (it does not walk parents). SavePreset uses this to refuse overwriting a
// user's hand-authored file; callers use it to warn before opening a save UI.
func HasLocalFile(dir string) bool {
	_, err := os.Stat(filepath.Join(dir, LocalFileName))
	return err == nil
}

// SavePreset writes p as the sole preset of a brand-new atelier.local.yaml in
// dir, under a primary-module ("." path) entry. It is deliberately create-only:
// if an atelier.local.yaml already exists in dir it returns an error wrapping
// fs.ErrExist and leaves the file untouched — Atelier never merges into or
// rewrites a file the user may have hand-authored (ADR-0026). To add further
// presets, users edit the file directly.
//
// The write is done with O_EXCL so the existence check is atomic; a header
// comment documenting provenance is prepended to the generated YAML.
func SavePreset(dir string, p Preset) error {
	m := &Manifest{Modules: []Module{{
		Path:    PrimaryModulePath,
		Presets: []Preset{p},
	}}}
