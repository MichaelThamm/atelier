...
| `tfvars.go` | Opt-in pass-through shape (ADR-0031) and `.tfvars` handling: `variables.tf` mirroring, forwarding `main.tf`, `terraform.tfvars` read/write, binding diagnostics, mode detection, and `PresetsDir`. |
